...
\begin{algorithm}
	...
	\begin{algorithmic}[1] % The number tells where the line numbering should start
		\Procedure{flattenLightBVHTree}{LinearLightBVHNode *nodes, LightBVHNode *node, int *offset}
...
The effect of calling this method on the root of the light BVH tree can be seen in \ref{TODO}. Suppose we have a very simple tree with only five total nodes consisting of two interior nodes and three leaf nodes like in the drawing. In our pre-flattened version of our light BVH, the nodes do not necessarily need to be in a contiguous chunk of memory. Instead, every interior node would have references to two child nodes. After the flattening, the nodes are ordered in a linear way in a coherent chunk of memory. The left child is implicitly referenced by the next node in the memory and the right child can be referenced via an offset. Obviously, with the non-linear implementation, our cache, memory and overall system performance could be extremely lacking in worst cases and will definitely be worse in average cases than our flattened version.
